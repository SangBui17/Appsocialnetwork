...
namespace Social_network.ServicesImp
{
    class PostService : PostRepository
    {
        ...
        public async Task<List<PostResponse>> getAllPostByMe(PageInfo pageinfo)
        {
            var client = new HttpClient();
            var serializerOptions = new JsonSerializerOptions
            {
            ...
            string url = $"http://10.0.2.2:2711/post/all/me";
            try
            {
                // Serialize PageInfo using System.Text.Json
                string json = System.Text.Json.JsonSerializer.Serialize(pageinfo, serializerOptions);
        ...
        public async Task<List<PostResponse>> getAllPostById(PageInfo pageinfo, long id)
        {
            var client = new HttpClient();
            var serializerOptions = new JsonSerializerOptions
            {
            ...
            string url = $"http://10.0.2.2:2711/post/all/{id}";
            try
            {
                // Serialize PageInfo using System.Text.Json
                string json = System.Text.Json.JsonSerializer.Serialize(pageinfo, serializerOptions);
