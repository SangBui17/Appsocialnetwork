...
{
	internal class NotificationService : NotificationRepository
	{
		public async Task<List<NotificationResponse>> getAllNotificattion(PageInfo pageInfo)
		{
			...
			string url = $"http://10.0.2.2:2711/notification";
			...
			try
			{
				...
				if (responseMessage.IsSuccessStatusCode)
				{
					...
					NotificationResponseWrapper resposeWrapper = JsonConvert.DeserializeObject<NotificationResponseWrapper>(responseContent);
					List<NotificationResponse> messageResponses = resposeWrapper.content;
